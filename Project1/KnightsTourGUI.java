...
public class KnightsTourGUI extends JPanel {
    ...
    static KnightsTourGUI panel;

    public static void main(String[] args) {
        // Setup GUI window
        JFrame frame = new JFrame("Knight's Tour Path");
        panel = new KnightsTourGUI();
        frame.add(panel);
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Draw knight as a green circle at the latest position
        if (!path.isEmpty()) {
            Point last = path.get(path.size() - 1);
            int cx = last.y * SIZE + SIZE / 2;
            int cy = last.x * SIZE + SIZE / 2;
